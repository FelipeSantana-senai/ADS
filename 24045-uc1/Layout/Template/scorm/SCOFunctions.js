...
const infosToCompleteScorm = {
  ...
  lastPageHTML: window.menuData.menu[window.menuData.menu.length - 1].page,
  lastSectionId: "finished"
};

const sections = document.querySelectorAll("section[id], article[id]");
const resumeFromLastButton = document.getElementById("resumeFromLastButton");
const resumeFromBeginning = document.getElementById("resumeFromBeginning");
...
resumeFromBeginning.addEventListener("click", () => {
  // Resetando dados ao começar do início
  doLMSSetValue("cmi.suspend_data", JSON.stringify({}));
  doLMSCommit();
...
// Função de debounce para o evento de scroll
  ...
  try {
    sections.forEach(section => {
      ...
      if (sectionTop < windowHeight && sectionTop > 0) {
        // Salva a rolagem e o ID da seção no suspend_data
        doLMSSetValue("cmi.suspend_data", JSON.stringify({ scrollY: section.id }));
        doLMSSetLessonLocation(window.location.href.split('#')[0]);
        doLMSCommit();

        // Marca como "completo" se atingir a última seção da última página
        if (infosToCompleteScorm.lastSectionId === section.id && window.location.href.includes(infosToCompleteScorm.lastPageHTML)) {
